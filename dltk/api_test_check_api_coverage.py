...
try:
    from .api_test_manifest_io import first_list, list_at, load_data
    ...
    def list_at(document: Any, key: str) -> list[dict[str, Any]]:
        ...
        return found
try:
    from .api_test_parse_openapi import extract, load_document
...
from .api_test_materialize_missing_bru import request_url as materialized_request_url
    ...
    for position, case in enumerate(cases, 1):
        ...
        if endpoint:
            ...
            if omitted_headers is not None and (
                ...
                if "dltk: omit-common-headers" not in contents[matched]:
        ...
        for step in case_database_steps:
            ...
            marker = "dltk: mock-data-prerequisite" if phase == "setup" else f"dltk: database-{phase} {engine}"
            if marker not in contents[matched]:
                errors.append(f"case {case_id} database {phase} step is not represented in Bruno")
        if case.get("mock_data_required") is True and "dltk: mock-data-prerequisite" not in contents[matched]:
...
def main() -> int:
    ...
    parser.add_argument("--allow-draft-version", action="store_true", help=argparse.SUPPRESS)
    parser.add_argument(
        "--execution-config",
        type=Path,
    ...
    if args.require_scenarios:
        ...
        version_command = [
                sys.executable,
                "-m", "dltk.api_test_check_version_compatibility",
                str(qa_root.parent),
                str(args.contracts_root),
                "--phase", "before-execute",
            ]
        if args.allow_draft_version:
            version_command.append("--allow-draft")
