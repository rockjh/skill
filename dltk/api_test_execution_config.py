...
from .api_test_qa_paths import BRUNO, CONTRACTS, EXECUTION, GLOBAL_EVIDENCE, LOGS, MODULE_EVIDENCE

RUNTIME_CONFIG_ENV = "__QA_EXECUTION_CONFIG"
DEFAULT_CLI_TIMEOUT = 60.0
COLLECTION_MARKER = "dltk: runtime-config"
COLLECTION_END_MARKER = "dltk: runtime-config-end"
TOOLING_MODES = {"shared-cli"}
...
exec dltk api-test run --qa-root "$SCRIPT_DIR/.." "$@"
"""
...
dltk api-test mock-data-generate --qa-root "%~dp0.." %*
exit /b %errorlevel%
"""
...
SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
exec dltk api-test mock-data-generate --qa-root "$SCRIPT_DIR/.." "$@"
"""
...
dltk api-test mock-data-clean --qa-root "%~dp0.." %*
exit /b %errorlevel%
"""
...
SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
exec dltk api-test mock-data-clean --qa-root "$SCRIPT_DIR/.." "$@"
"""
...
def validate_execution_config(document: Any) -> dict[str, Any]:
    ...
    if tooling not in TOOLING_MODES:
        raise ValueError("tooling must be shared-cli")
...
def initialize_execution_layout(qa_root: Path, local_scripts: bool | None = None) -> list[Path]:
    """Create reusable execution assets and lightweight shared-CLI launchers."""
