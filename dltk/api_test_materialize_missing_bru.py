...
from .api_test_parse_openapi import display_directory, render_manifest, update_module_document
...
LEGACY_AUTH_MARKER = "dltk: auth-start"
LEGACY_AUTH_END_MARKER = "dltk: auth-end"
OMIT_MARKER = "dltk: omit-common-headers"
OMIT_END_MARKER = "dltk: omit-common-headers-end"
...
def post_response_script(case: dict[str, Any], assertions: list[dict[str, Any]]) -> str:
    ...
    for capture in captures if isinstance(captures, list) else []:
        ...
        if expression:
            ...
            lines.extend([
                f"  test({json.dumps('dltk:flow:capture:' + str(capture['name']))}, function () {{",
    ...
    for name in case.get("flow_uses", []) if isinstance(case.get("flow_uses"), list) else []:
        if not str(name).strip():
            continue
        lines.extend([
            f"  test({json.dumps('dltk:flow:use:' + str(name))}, function () {{",
    ...
    if absent_path:
        ...
        if expression:
            lines.extend([
                f"  test({json.dumps('dltk:flow:absence:' + absent_path)}, function () {{",
    ...
    for step in database_steps(case):
        ...
        lines.append(f"  // dltk: database-assertion {engine}")
