...
from .api_test_command_execution import command_argv
from .api_test_materialize_missing_bru import request_url
from .api_test_mock_data import (
    MockDataError,
...
def finish_mock_data_run(
    ...
    try:
        ...
        print(f"Retry: dltk api-test mock-data-clean --qa-root {qa_root} --run-id {run_id}", file=sys.stderr)
        return 10
    if result.get("status") == "retained":
        print(f"Mock data retained. Cleanup: dltk api-test mock-data-clean --qa-root {qa_root} --run-id {run_id}")
    return 0
...
def execute(args: argparse.Namespace, qa_root: Path, execution_log: Path) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="qa-bruno-") as directory:
        ...
        preflight_command = [
            sys.executable,
            "-m", "dltk.api_test_runtime_preflight",
            "--execution-config", str(config_path),
        ...
        normalize_result = subprocess.run(
            [
                sys.executable,
                "-m", "dltk.api_test_normalize_bruno_report",
                str(raw_report),
