...
from .e2e_assets import initialize
from .e2e_contracts import generate_artifacts
from .e2e_discovery import PROTOCOL_SUFFIXES, discover_documents, discover_protocols, read_only_environment_probe
from .e2e_runner import ORDERED_GATES, check_gate, pytest_arg_errors, run_ordered
from .e2e_source_versions import RULES_VERSION, source_version_results


def init_command(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(prog="dltk e2e init")
...
def discover_command(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(prog="dltk e2e discover")
    parser.add_argument("--project", type=Path, default=Path("."))
    parser.add_argument("--design-root", action="append", type=Path, default=[])
    parser.add_argument("--design-file", action="append", type=Path, default=[])
    parser.add_argument("--openapi-root", action="append", type=Path, default=[])
    ...
    if args.runtime_urls:
        from .e2e_discovery import read_only_protocol_probe
...
def generate_command(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(prog="dltk e2e generate")
    parser.add_argument("--project", type=Path, default=Path("."))
    parser.add_argument("--design-root", action="append", type=Path, default=[])
    parser.add_argument("--design-file", action="append", type=Path, default=[])
    parser.add_argument("--openapi-root", action="append", type=Path, default=[])
...
def check_command(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(prog="dltk e2e check")
    parser.add_argument("--project", type=Path, default=Path("."))
...
def source_status_command(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(prog="dltk e2e source-status")
    parser.add_argument("--project", type=Path, default=Path("."))
    parser.add_argument("--scenario")
    args = parser.parse_args(argv)
...
def run_command(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(prog="dltk e2e run")
    parser.add_argument("--project", type=Path, default=Path("."))
    parser.add_argument("--scenario")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="dltk e2e")
