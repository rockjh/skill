...
from .e2e_source_versions import build_generation_lock, generation_lock_errors, input_summary
from .e2e_context import build_context, classify_rule
from .e2e_materialize import materialize_scenarios
...
def _runtime_protocol_documents(project_root: Path) -> tuple[list[Path], dict[str, Any]]:
    ...
    if not urls:
        from .e2e_discovery import read_only_environment_probe
    ...
    for item in probe.get("documents", []):
        ...
        with tempfile.NamedTemporaryFile("w", encoding="utf-8", suffix=suffix, prefix="dltk-e2e-runtime-", delete=False) as stream:
...
def _generation_artifact_errors(project_root: Path, definition: Mapping[str, Any]) -> list[str]:
    ...
    for path in required_artifacts:
        if not path.is_file():
            errors.append(_error(path, "generation-artifact-required", "缺少设计权威生成产物；必须先运行 dltk e2e generate"))
