...
from .schema import E2E_GATE_SCHEMA_VERSION
from .e2e_discovery import SHA_RE, _git, _resolve
...
def build_generation_lock(
    ...
    *,
    ...
    return {
        "version": 1,
        "tool": {"name": "dltk", "version": __version__, "e2e_schema": E2E_GATE_SCHEMA_VERSION},
...
def generation_lock_errors(project_root: Path, lock: Any) -> list[str]:
    ...
    tool = lock.get("tool")
    if not isinstance(tool, dict) or tool.get("name") != "dltk" or tool.get("e2e_schema") != E2E_GATE_SCHEMA_VERSION:
...
def source_version_results(project_root: Path, selected: str | None = None) -> tuple[list[str], list[dict[str, Any]]]:
    ...
    from .e2e_contracts import contract_errors
