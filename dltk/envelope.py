...
def failure(command: str, error: DltkError) -> dict[str, Any]:
    return redact({
        "ok": False,
        "tool": "dltk",
        "command": command,
...
def render(document: dict[str, Any], *, markdown: bool) -> str:
    ...
    if document["ok"]:
        lines = [f"# dltk {document['command']}", "", "Status: OK"]
    ...
    lines = [f"# dltk {document['command']}", "", f"Status: {error['code']}", "", error["message"]]
    if error.get("hint"):
