...
database_steps:
  - phase: setup
    ...
    ownership:
      ...
      resource: thing
      selector: id = DLTK_DATA_NAMESPACE
    precheck: |
      ...
      const [rows] = await connection.execute(
        ...
        [bru.getEnvVar("DLTK_DATA_NAMESPACE")]
      );
      await connection.end();
      bru.setVar("DLTK_STEP_EXISTS", rows.length ? "true" : "false");
    script: |
      const mysql = require("mysql2/promise");
      const connection = await mysql.createConnection({
        host: bru.getEnvVar("MYSQL_HOST"),
      ...
      const thingId = bru.getEnvVar("DLTK_DATA_NAMESPACE");
    ...
    cleanup: |
      ...
      await connection.execute("DELETE FROM thing WHERE id = ?", [bru.getEnvVar("DLTK_DATA_NAMESPACE")]);
      await connection.end();
