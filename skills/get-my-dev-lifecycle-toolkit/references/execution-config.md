...
The generated launchers are thin convenience assets and call the installed `dltk api-test` commands; they contain no Python implementation. Run launchers cover tests plus standalone mock-data generation and cleanup on CMD and Shell. `dltk api-test scripts` reports the installed domain schema and does not synchronize files.
