...
```bash
dltk api-test worker-start --module <module>
```
...
5. Run `dltk api-test run --module <module>`. It validates the module lock and writes module-local evidence, results, and logs.
6. Run `dltk api-test worker-check --module <module> --stage post-execution`. Changed paths are calculated from the recorded snapshot.
...
6. runs `dltk api-test aggregate` to reconcile independent module reports and evidence; and
