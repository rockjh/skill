...
def _run_domain(domain: str, arguments: list[str], *, full: bool) -> tuple[dict[str, object], int]:
    if not arguments:
        raise DevAIError("INVALID_ARGUMENT", f"missing {domain} command", ExitCode.ARGUMENT)
...
def console_main(argv: list[str] | None = None) -> int:
    ...
    try:
        if command == "version":
            if len(arguments) != 1:
                raise DevAIError("INVALID_ARGUMENT", "version accepts no arguments", ExitCode.ARGUMENT)
            ...
            if len(arguments) != 1:
                raise DevAIError("INVALID_ARGUMENT", "doctor accepts no arguments", ExitCode.ARGUMENT)
            ...
            if healthy:
                ...
                error = DevAIError(
                    ...
                    "Install the missing requirements and run dev-ai doctor again.",
                )
                document = failure("doctor", error)
                code = ExitCode.UNAVAILABLE
        elif command == "schema":
            if len(arguments) > 2:
                raise DevAIError("INVALID_ARGUMENT", "schema accepts at most one scope", ExitCode.ARGUMENT)
            scope = arguments[1] if len(arguments) == 2 else None
            try:
                data = get_schema(scope)
            except KeyError as exc:
                raise DevAIError("INVALID_ARGUMENT", f"unknown schema scope: {scope}", ExitCode.ARGUMENT) from exc
            ...
            raise DevAIError("INVALID_ARGUMENT", f"unknown command: {command}", ExitCode.ARGUMENT)
    except DevAIError as exc:
        document = failure(command, exc)
        code = exc.exit_code
    except Exception as exc:
        error = DevAIError("INTERNAL_ERROR", str(exc) or exc.__class__.__name__, ExitCode.INTERNAL)
