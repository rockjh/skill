...
from .constraints import database_access_errors, mock_data_ready_env
from .manifest_io import first_list, load_data
from .qa_paths import BRUNO, CONSTRAINTS, CONTRACTS, EXECUTION


INVENTORY_FILE = "mock-data.yaml"
MODULE_PLAN_FILE = "mock-data.yaml"
RESULT_DIRECTORY = Path("results") / "mock-data"
RUN_NAMESPACE_ENV = "DEV_AI_DATA_NAMESPACE"
READY_ENV = "DEV_AI_MOCK_DATA_READY"
STEP_EXISTS_ENV = "DEV_AI_STEP_EXISTS"
SCRIPT_RESULT_MARKER = "__DEV_AI_MOCK_DATA_RESULT__"
...
def _api_script_prelude() -> str:
    return r"""const namespace = bru.getEnvVar('DEV_AI_DATA_NAMESPACE');
const id = bru.getEnvVar('__ID_ENV__');
const baseUrl = bru.getEnvVar('baseUrl') || bru.getEnvVar('BASE_URL');
const headers = JSON.parse(bru.getEnvVar('DEV_AI_MOCK_DATA_HEADERS') || '{}');
...
def _api_step(
    ...
    variable = f"DEV_AI_DATA_{re.sub(r'[^A-Za-z0-9]+', '_', str(entity['name'])).strip('_').upper()}_ID"
...
def _sql_step(
    ...
    namespace = "const namespace = bru.getEnvVar('DEV_AI_DATA_NAMESPACE');"
    if engine in {"mysql", "mariadb"}:
...
def derive_mock_data_contracts(qa_root: Path) -> list[Path]:
    ...
    variable_by_entity = {
        str(entity.get("name")): f"DEV_AI_DATA_{re.sub(r'[^A-Za-z0-9]+', '_', str(entity.get('name'))).strip('_').upper()}_ID"
...
def is_protected_environment(name: str) -> bool:
    ...
    configured = {
        value.strip().casefold() for value in os.environ.get("DEV_AI_PROTECTED_ENVIRONMENTS", "").split(",") if value.strip()
    }
...
def _runtime_variables(qa_root: Path, namespace: str) -> dict[str, str]:
    ...
    variables["DEV_AI_MOCK_DATA_HEADERS"] = json.dumps(
...
def _execute_script(qa_root: Path, script: str, variables: dict[str, str], label: str) -> dict[str, str]:
    ...
    runner = """const values = JSON.parse(process.env.DEV_AI_MOCK_DATA_ENV || '{}');
const changed = {};
global.bru = {
...
""".replace("__SCRIPT__", script).replace("__RESULT__", SCRIPT_RESULT_MARKER)
    ...
    path: Path | None = None
    try:
        with tempfile.NamedTemporaryFile("w", suffix=".cjs", prefix=".dev-ai-mock-data-", dir=bruno_root, delete=False, encoding="utf-8") as handle:
            handle.write(runner)
            path = Path(handle.name)
        environment = dict(os.environ)
        environment["DEV_AI_MOCK_DATA_ENV"] = json.dumps(variables, ensure_ascii=False)
    ...
    safe = {
        ...
        if str(name).startswith(("DEV_AI_DATA_", "DEV_AI_STEP_")) and len(str(value)) <= 4096
    }
    ...
    *,
    ...
    for step in plan["steps"]:
        ...
        for name, definition in definitions.items() if isinstance(definitions, dict) else []:
            if not str(name).startswith("DEV_AI_DATA_") or not isinstance(definition, dict):
    ...
    ledger["runtime_variables"] = {
        name: value for name, value in variables.items() if name.startswith("DEV_AI_DATA_")
    }
    _write_ledger(path, ledger)
    ...
    def persist_owned_runtime_variables() -> None:
        ledger["runtime_variables"] = {
            ...
            if name.startswith(("DEV_AI_DATA_", "DEV_AI_MOCK_DATA_READY"))
        }
        _write_ledger(path, ledger)
...
def cleanup(
    ...
    *,
    ...
    variables.update({
        ...
        if str(name).startswith("DEV_AI_DATA_")
