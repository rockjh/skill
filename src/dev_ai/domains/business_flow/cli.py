...
def _write_scope_error(project: Path, docs_root: Path) -> str | None:
    ...
    for name in ("DEV_AI_ENV", "APP_ENV", "DEPLOY_ENV", "RUNTIME_ENV", "ENVIRONMENT"):
    ...
    if os.environ.get("DEV_AI_PROTECTED", "").strip().casefold() in {"1", "true", "yes", "on"}:
        return "business-flow writes are blocked by DEV_AI_PROTECTED"
    return None
