...
class RegressionTests(unittest.TestCase):
    ...
    def test_empty_flow_skips_without_flow_requirement_but_blocks_explicit_flow(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            skipped = subprocess.run(
                [*module_command("validate_flow_execution"), str(flows), "--results", str(results)],
                check=False,
                capture_output=True,
                text=True,
                encoding="utf-8",
            ...
            blocked = subprocess.run(
                [*module_command("validate_flow_execution"), str(flows), "--endpoints", str(endpoints), "--results", str(results)],
                check=False,
                capture_output=True,
                text=True,
                encoding="utf-8",
    ...
    def test_partition_writes_security_profile_and_draft_index(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            materialized = subprocess.run(
                [
                    *module_command("materialize_missing_bru"),
                    str(root / "contracts"),
                    str(root / "bruno"),
                ],
                check=False,
            ...
            checked = subprocess.run(
                [
                    *module_command("check_api_coverage"),
                    str(root / "contracts"),
                    str(root / "bruno"),
                    "--all",
                    "--json",
    ...
    def test_optional_common_header_does_not_block_preflight(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(
                [
                    *module_command("runtime_preflight"),
    ...
    def test_version_lock_can_initialize_a_draft_baseline(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(
                [*module_command("check_version_compatibility"), str(repo), str(contracts), "--init", "--json"],
                check=False,
                capture_output=True,
                text=True,
                encoding="utf-8",
            ...
            blocked = subprocess.run(
                [
                    *module_command("check_version_compatibility"),
    ...
    def test_source_scanner_cli_blocks_unmapped_coverage_candidate(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(
                [
                    *module_command("analyze_source_logic"),
                    str(source),
    ...
    def test_coverage_without_execution_is_draft_not_complete(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(
                [*module_command("check_api_coverage"), str(contracts), str(root / "bruno"), "--module", "things", "--json"],
                check=False,
                capture_output=True,
                text=True,
                encoding="utf-8",
            ...
            self.assertEqual(verified_report["status"], "draft")
            self.assertEqual(verified_report["module_status"], "failed")
    ...
    def test_module_execution_reports_verified_without_changing_global_state(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            contracts = qa_root / "contracts" / "things"
            bru_root = qa_root / "bruno"
            bru = bru_root / "things"
    ...
    def test_generated_project_does_not_copy_dev_ai_sources(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual("shared-cli", execution_config.load_execution_config(qa_root / "execution" / "config.yaml")["tooling"])
    ...
    def test_public_cli_pipeline_uses_only_the_canonical_layout(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(cli.scripts_command([]), 0)
            self.assertFalse((qa_root / "scripts").exists())
