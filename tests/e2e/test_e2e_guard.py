...
for _module in (DISCOVERY, CONTRACTS, STATIC_CHECKS, RUNNER):
    for _name in dir(_module):
        if not _name.startswith("__"):
            setattr(GUARD, _name, getattr(_module, _name))
...
class StaticGuardTests(unittest.TestCase):
    ...
    def test_discovery_rejects_declared_build_without_module(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            errors, _ = DISCOVERY.discovery_errors(root)
    ...
    def test_gate_seal_detects_changed_input(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            RUNNER._start_gate_session(root)
            RUNNER._write_seal(root, "discovery", [source])
            self.assertEqual([], RUNNER._seal_errors(root, "discovery", [source]))

            source.write_text("value: 2\n", encoding="utf-8")
            self.assertTrue(any("gate-order" in error for error in RUNNER._seal_errors(root, "discovery", [source])))
    ...
    def test_project_local_tool_launchers_are_rejected(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            scripts.mkdir()
            (scripts / "run.py").write_text("print('copied tool')\n", encoding="utf-8")
            self.assertTrue(any("project-tool-source-forbidden" in error for error in GUARD._launcher_errors(root)))
    ...
    def test_legacy_runtime_config_name_is_rejected(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            errors = STATIC_CHECKS.static_errors(root)
    ...
    def test_runner_defaults_to_all_and_accepts_scenario_name(self) -> None:
        ...
        with patch.object(E2E_CLI, "run_ordered", return_value=0) as run:
            self.assertEqual(0, E2E_CLI.run_command([]))
            self.assertIsNone(run.call_args.args[1])

            self.assertEqual(0, E2E_CLI.run_command(["--scenario", "场景"]))
    ...
    def test_static_only_runner_writes_ordered_na_report(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                ...
                patch.object(RUNNER, "contract_errors", return_value=([], [], {})),
            ):
                result = RUNNER.run_ordered(root, None, [], static_only=True)
    ...
    def test_runner_stops_after_first_failed_gate(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            with (
                patch.object(RUNNER, "_run", return_value=7) as run,
                patch.object(RUNNER, "contract_errors", return_value=([], [], {})),
            ):
    ...
    def test_runner_attributes_second_discovery_stage_failure(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            with (
                patch.object(RUNNER, "_run", side_effect=[0, 7]),
                patch.object(RUNNER, "contract_errors", return_value=([], [], {})),
            ):
                result = RUNNER.run_ordered(root, None, [], static_only=False)
    ...
    def test_selected_contract_still_checks_sibling_collisions(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                patch.object(CONTRACTS, "discovery_errors", return_value=([], {})),
                patch.object(CONTRACTS, "_scenario_errors", return_value=[]),
            ):
                errors, selected, _ = CONTRACTS.contract_errors(root, "甲")
    ...
    def test_ordered_runtime_probe_rechecks_live_process_listener_and_http(self) -> None:
        ...
        with (
            patch.object(DISCOVERY, "_observed_process_command", return_value="python service-ref"),
    ...
    def test_runner_does_not_reuse_evidence_created_before_a_scenario(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                patch.object(RUNNER.uuid, "uuid4") as uuid4,
                patch.object(RUNNER, "_run", side_effect=run),
                patch.object(RUNNER, "source_version_results", return_value=([], [])),
                patch.object(RUNNER, "contract_errors", return_value=contract_result),
                patch.object(RUNTIME, "preflight", return_value={}),
    ...
    def test_runner_keeps_restoration_na_when_preflight_fails(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            contract_result = ([], [(scenario, definition)], {"runtime_probe": {"requested": False}})
            with (
                patch.object(RUNNER, "_run", return_value=0),
                patch.object(RUNNER, "source_version_results", return_value=([], [])),
                patch.object(RUNNER, "contract_errors", return_value=contract_result),
                patch.object(RUNTIME, "preflight", side_effect=RuntimeError("preflight")),
            ):
                result = RUNNER.run_ordered(root, None, [], static_only=False)
    ...
    def test_aggregate_gates_do_not_write_ordering_seals(self) -> None:
        ...
        with (
            patch.object(RUNNER, "discovery_errors", return_value=([], {})),
            patch.object(RUNNER, "contract_errors", return_value=([], [], {})),
            patch.object(RUNNER, "_write_seal") as write_seal,
        ):
            self.assertEqual([], RUNNER.check_gate(Path("."), "discovery"))
            self.assertEqual([], RUNNER.check_gate(Path("."), "contracts"))
    ...
    def test_read_only_rpc_contract_accepts_one_bounded_returned_call(self) -> None:
        ...
        with patch.object(STATIC_CHECKS, "_source_reference_exists", return_value=True):
    ...
    def test_gate_session_rejects_replay_and_out_of_order_stage(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            source.write_text("value: 1\n", encoding="utf-8")
            RUNNER._start_gate_session(root)
            RUNNER._write_seal(root, "workspace_inventory", [source])
            self.assertTrue(RUNNER._gate_session_errors(root, "dependency_topology"))
            RUNNER._start_gate_session(root)
            self.assertTrue(RUNNER._seal_errors(root, "workspace_inventory", [source]))
    ...
    def test_source_contract_requires_discovery_commit_and_all_relevant_repositories(self) -> None:
        ...
        with patch.object(CONTRACTS, "_git", return_value=subprocess.CompletedProcess([], 0, "", "")):
